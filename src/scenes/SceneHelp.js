...
export default class SceneHelp extends Phaser.Scene {
    ...
    create() {
        ...
        var exitButton = this.add.rectangle(40.5, 38.5, 40, 40, 0x000000)
        //var previousPage = this.add.rectangle(165, 740, 20, 20, 0x000000) 
        var nextPage = this.add.rectangle(358, 740, 40, 40, 0x000000)
        var previousPage = this.add.rectangle(165, 740, 40, 40, 0x000000)
        ...
        var previousButton = this.plugins.get('rexbuttonplugin').add(previousPage, {
            enable: true,
            mode: 0,            // 0|'press'|1|'release'
            // clickInterval: 100  // ms
        });
        previousButton.on('click', function (button, gameObject, pointer, event) {
            //  calledBack = true;
            //countdown = 1
            if (this.onPage > 1) {
                this.onPage -= 1
        ...
        var nextButton = this.plugins.get('rexbuttonplugin').add(nextPage, {
            enable: true,
            mode: 0,            // 0|'press'|1|'release'
            // clickInterval: 100  // ms
        });
        nextButton.on('click', function (button, gameObject, pointer, event) {
            //  calledBack = true;
            //countdown = 1
            if (this.onPage < 3) {
                this.onPage += 1
